...
const CreatePost = () => {
  ...
  const [title, setTitle] = useState('');
  const [subject, setSubject] = useState('');
  const [writerName] = useState(user.UserName || 'אנונימי'); // לא ניתן לשנות ידנית
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  ...
  const handlePost = async () => {
    ...
    try {
      await dispatch(addPost({
        ...
        subject,
        writerName,
        userId: user._id  // 🔥 התיקון הקריטי: שליחת userId לשרת
      })).unwrap();
  ...
  return (
    <Box sx={{ maxWidth: 500, margin: '20px auto', textAlign: 'center' }}>
      ...
      <Button
        variant="contained"
        onClick={handlePost}
        disabled={!title.trim() || !subject.trim() || loading}
      >
        {loading ? 'טוען...' : 'פרסם פוסט'}
      </Button>
    </Box>
